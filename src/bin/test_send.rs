...
fn main_error() -> Result<(), anyhow::Error> {
    ...
    let usage = format!(
        "{}\n{}\n{}",
        "用法:",
        format!("    {} iface_name gateway_mac - 测试不同的发送方案", app_name),
    ...
    // 匹配接口名称
    let (iface, src_ip) = get_iface_by_name(&if_name).ok_or_else(|| raise_error!("raw", __func__, "查不到指定接口"))?;
    let src_ip = src_ip.ok_or_else(|| raise_error!("raw", __func__, "指定接口查不到 IPv4 地址"))?;
...
fn send_l2(
    ...
    let (mut tx, mut _rx) = create_l2_channel(iface).or_else(|err| raise_error!(__func__, "\n", err))?;

    let src_mac = iface
        .mac
        .ok_or_else(|| raise_error!("raw", __func__, "无法获取源 MAC 地址"))?;
    let dst_mac = parse_mac(gw_mac);
    let dst_ip = Ipv4Addr::new(rng.gen::<u8>(), rng.gen::<u8>(), rng.gen::<u8>(), rng.gen::<u8>());
...
fn send_l4(src_ip: &Ipv4Addr, mut rng: &mut ThreadRng) -> Result<(), anyhow::Error> {
    ...
    let mut tx_packet =
        MutableTcpPacket::new(&mut tx_buffer).ok_or_else(|| raise_error!("raw", __func__, "无法创建 TcpPacket"))?;
    build_l4_tcp_packet(&mut tx_packet, &src_ip, &dst_ip, src_port, dst_port, &mut rng);
